...
export type CanonicalFilters = Partial<
  Record<CohortKey, string | number | boolean>
>;
...
export function canonicalFilters(f: WorkFilters): CanonicalFilters {
  ...
  if (out.label_source !== "classifier") {
    delete out.label_mode;
    delete out.classified;
    delete out.classifier_version;
  } else {
...
function canonicalString(c: CanonicalFilters): string {
  const sorted = Object.fromEntries(
    Object.entries(c).sort(([a], [b]) => (a < b ? -1 : 1)),
  );
...
export function hashFilters(f: WorkFilters): string {
  return createHash("sha256")
    .update(canonicalString(canonicalFilters(f)))
    .digest("hex")
    .slice(0, 12);
}
...
export async function savePermalink(
  f: WorkFilters,
): Promise<{ hash: string; filters: CanonicalFilters }> {
