...
| 28 | **The rubric is silent on 89% of the works the models fought over.** AN INDEPENDENT BLINDED JUDGE SAYS THE RUBRIC IS SILENT ON 89% OF THE WORKS THE MODELS FOUGHT OVER. Fable 5, which is not one of the three screened arms, adjudicated all 179 contested works seeing three screener opinions as A/B/C in random order with no model names, so it was asked which reading of the RUBRIC is right rather than which model to trust. It shows no arm preference (spread 1.16x, chi-square p = 0.493), so it is not a fourth vote for one of the screeners, and it overruled ALL THREE on 6 works. Its verdict on the instrument: the rubric is SILENT on 159 of 179 contested works, and only 17 splits are a screener misapplying a rule that actually exists. THE FIELD'S BOUNDARY IS BEING SET BY THE SCREENER, NOT BY THE INSTRUMENT. The seams that cost the most agreement are methods_dev_vs_study (39 works), lis_sts_asymmetry (26), and history_of_science (19). This is what converts adjudication from a tiebreak into a measurement: not 179 answers, but a frequency-weighted census of which sentences the rubric is missing and exactly how many works each one costs. The judge's TIERS are used for nothing and produce no estimate: a model cannot certify a model, and that argument applies to the judge too. | The competition asks what criteria can be used for inclusion and exclusion. This is the answer, and it is a measurement rather than an opinion. The obvious move, having Opus adjudicate the disagreements between Opus, GPT and Grok, is disqualifying: Opus IS one of the arms, so one of the three votes would be the court. So the judge is FABLE 5, which is not one of the arms, and it is BLIND: it sees the three screener opinions as A, B and C in a random order per work, with no model names anywhere, so it is asked which reading of the RUBRIC is right rather than which model to trust. The check that could have falsified the whole design was run rather than waved away: the judge shows NO ARM PREFERENCE (agrees with opus 53%, gpt 53%, grok 61%; spread 1.16x, chi-square p = 0.49), so it is not a fourth vote for one of the screeners, and it overruled all three on 6 works. ITS VERDICT ON THE INSTRUMENT: the rubric is SILENT on 159 of the 179 contested works (89%), and only 17 of the splits are a screener misapplying a rule that actually exists. THE FIELD'S BOUNDARY IS BEING SET BY THE SCREENER, NOT BY THE INSTRUMENT. The seams that cost the most agreement, as a frequency-weighted census: methods_dev_vs_study (39 works), lis_sts_asymmetry (26), history_of_science (19), workforce_boundary (19). Two judges on disjoint batches, with no contact, independently named the same top seam AND independently proposed nearly the same missing sentence. Consequence: rubric v2 is not a wish list, it is written against a census of what the ambiguity actually costs, and the judge's TIERS are used for nothing and produce no estimate, because a model cannot certify a model and that argument applies to the judge too. |
| 29 | **Writing the missing sentences resolved 54% of the disagreements.** WRITING THE MISSING SENTENCES RESOLVED 54% OF THE DISAGREEMENTS THEY WERE WRITTEN AGAINST. The 179 works three frontier models split on under rubric v1 were re-screened, by the same three models, under a v2 whose rules were written against a published, frequency-weighted census of WHICH SENTENCES THE RUBRIC WAS MISSING. 96 of 179 are now unanimous, and the pairwise Jaccard of the in-scope sets moves from 0.19 to 0.36. This is a measurement almost nobody makes, and it is available ONLY because the ambiguity was enumerated, adjudicated and PUBLISHED BEFORE it was resolved: the census went out first, so v2 could not be quietly tuned until the numbers looked good. SCOPE, stated rather than blurred: these works were SELECTED FOR DISAGREEMENT, so this says the rules resolve the cases they were written for; it does NOT say frame-wide agreement rose, because regression to the mean alone would move a set selected this way. The unbiased test is a full re-screen of all 5,600 under v2, prespecified, and it is what the funded work runs. | The measurement almost nobody makes, and it is only available because the ambiguity was PUBLISHED BEFORE IT WAS RESOLVED. The sequence: lock v1; screen 5,600 works with three models; have an independent blinded judge adjudicate every disagreement AND NAME THE SEAM that caused it; publish that frequency-weighted census; write v2 against the census and lock it BEFORE re-screening; re-screen; report the difference, whatever it turns out to be. Publishing the census first is what stops v2 being quietly tuned until the numbers improve. Result: of the 179 works three frontier models split on under v1, 96 (54%) are UNANIMOUS under v2, and the pairwise Jaccard of the in-scope sets moves from 0.19 to 0.36. Two rules carry most of it: 'developing, deriving, validating or teaching a technique for a domain is OUT; only methods AS PRACTICED are T1', and 'the STS, LIS and history bullets apply only where the object is science or research, at any period' (which also DELETES the word contemporary, an unexamined adverb that was silently excluding the entire history of Canadian science). SCOPE, stated rather than blurred: these 179 works were SELECTED FOR DISAGREEMENT, so this says the rules resolve the cases they were written for; it does NOT say frame-wide agreement rose, because regression to the mean alone would move a set selected this way. The unbiased test is a full re-screen of all 5,600 under v2, prespecified, and it is what the funded work runs. Consequence: the residual 83 works are the field's genuinely contested core, and the screening agent's report on v2 already names four classes v2 is still silent on and three answers it gets WRONG, which is the v2.2 agenda and is published with it. |
