 ...
 *
 * Colours come from the CSS custom properties, so every chart is correct in both
...
const ChartAccessibility = createContext<{
  title: string;
  titleId: string;
} | null>(null);
 ...
 * underneath is left with no dimensions, and Recharts silently renders NOTHING,
...
export function Frame({
  ...
  const titleId = useId();
  return (
    <ChartAccessibility.Provider value={{ title, titleId }}>
      <section className="card p-5">
        <h3 id={titleId} className="font-serif text-lg">{title}</h3>
        {note && (
          <p
...
function Responsive({
  ...
  height: number;
  summary: string;
  children: React.ReactElement<{ accessibilityLayer?: boolean }>;
}) {
  const accessible = useContext(ChartAccessibility);
  const summaryId = useId();
  const chart = cloneElement(children, { accessibilityLayer: false });
  return (
    <>
      <div
        role="img"
  ...
  return (
    <Responsive
      ...
      summary={data
        .map(
          (point) =>
            `${point.year}: ${t.charts.allWorks} ${point.works.toLocaleString(locale)}, ${t.charts.noCaAff} ${point.no_aff.toLocaleString(locale)}`,
        )
        .join("; ")}
    >
...
export function ByRouteChart({
  ...
  const t = getDict(lang);
  const locale = numberLocale(lang);
  const rows = data.marginals.map((m) => ({
    ...m,
    label: routeLabel(lang, m.route, m.label),
  }));
  return (
    <Responsive
      height={height}
      summary={rows
        .map(
          (row) =>
            `${row.label}: ${row.works.toLocaleString(locale)} ${t.charts.works}`,
        )
        .join("; ")}
    >
      <BarChart
      ...
      >
        ...
        />
        <Tooltip
          content={<ChartTip locale={locale} />}
...
export function RouteOverlapChart({
  data,
  lang,
  height = 260,
}: {
  ...
  return (
    <Responsive
      ...
      summary={top
        .map(
          (row) =>
            `${row.combo}: ${row.works.toLocaleString(locale)} ${t.charts.works}`,
        )
        .join("; ")}
    >
      <BarChart
  ...
  return (
    <Responsive
      ...
      summary={top
        .map(
          (row) =>
            `${row.field}: ${row.works.toLocaleString(locale)} ${t.charts.works}`,
        )
        .join("; ")}
    >
      <BarChart
  ...
  return (
    <Responsive
      ...
      summary={top
        .map(
          (row) =>
            `${row.lang}: ${row.works.toLocaleString(locale)} ${t.charts.works}`,
        )
        .join("; ")}
    >
      <BarChart data={top} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
        ...
        <Bar dataKey="works" name={t.charts.works} radius={[4, 4, 0, 0]}>
          {top.map((l, i) => (
            ...
            <Cell
              key={i}
              fill={l.lang === "fr" ? "var(--mc-accent)" : "var(--mc)"}
            />
          ))}
        </Bar>
      </BarChart>
 ...
 * the finding is that the gap is structural: it is concentrated in types that
 * never carry an abstract, rather than uniform noise a better index would fix.
 */
export function AbstractGapChart({
  data,
  lang,
  height = 400,
}: {
  ...
  const sorted = [...data].sort(
    (a, b) => b.pct_no_abstract - a.pct_no_abstract,
  );
  return (
    <Responsive
      height={height}
      summary={sorted
        .map(
          (row) =>
            `${row.type}: ${row.pct_no_abstract.toLocaleString(locale)}% ${t.charts.noAbstract}`,
        )
        .join("; ")}
    >
      <BarChart
      ...
      >
        ...
        />
        <Tooltip
          content={<ChartTip suffix="%" locale={locale} />}
          cursor={{ fill: "var(--surface-2)" }}
        />
        <Bar
          dataKey="pct_no_abstract"
          name={t.charts.noAbstract}
          fill="var(--contested)"
          radius={[0, 4, 4, 0]}
        />
      </BarChart>
  ...
  return (
    <Responsive
      ...
      summary={top
        .map(
          (row) =>
            `${row.venue}: ${row.works.toLocaleString(locale)} ${t.charts.works}`,
        )
        .join("; ")}
    >
      <BarChart
  ...
  return (
    <Responsive
      ...
      summary={top
        .map(
          (row) =>
            `${row.funder}: ${row.works.toLocaleString(locale)} ${t.charts.works}`,
        )
        .join("; ")}
    >
      <BarChart
  ...
  return (
    <Responsive
      ...
      summary={data
        .map(
          (row) =>
            `${row.nature}: ${t.charts.flagged} ${row.openalex_flagged.toLocaleString(locale)}, ${t.charts.missed} ${row.openalex_missed.toLocaleString(locale)}`,
        )
        .join("; ")}
    >
      <BarChart data={data} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
        ...
        <Legend wrapperStyle={{ fontSize: 12 }} />
        <Bar
          dataKey="openalex_flagged"
          name={t.charts.flagged}
          stackId="a"
          radius={[0, 0, 0, 0]}
        >
          {data.map((d, i) => (
