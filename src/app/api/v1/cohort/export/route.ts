 ...
 *
 * Every row carries every works column, sparse direct model labels with their
 * agreement, unvalidated frame prediction signals, provisional scores, and
 * the per-row status fields verbatim. An empty labels field means UNLABELED,
...
const CSV_COLUMNS = [
  ...
  'prediction_candidate_categories',
  'prediction_consensus_categories',
  'prediction_status',
  'prediction_teacher_disagreement_score',
  'prediction_threshold_uncertainty_score',
  'prediction_json',
] as const
...
function csvRow(r: ExportRow): string {
  ...
  const cells: Record<(typeof CSV_COLUMNS)[number], unknown> = {
    ...
    prediction_model_version: r.prediction?.model_version ?? '',
    prediction_candidate_categories: r.prediction?.candidate_categories.join(';') ?? '',
    prediction_consensus_categories: r.prediction?.consensus_categories.join(';') ?? '',
    prediction_status: r.prediction?.prediction_status ?? '',
    prediction_teacher_disagreement_score: r.prediction?.teacher_disagreement_score ?? '',
    prediction_threshold_uncertainty_score: r.prediction?.threshold_uncertainty_score ?? '',
...
export async function GET(req: NextRequest) {
  ...
  const meta = {
    ...
    truncated,
    label_status: 'direct model label, unvalidated',
