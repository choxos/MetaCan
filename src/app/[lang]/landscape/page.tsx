...
} from "@/components/CollaborationNetwork";
import collaborationData from "@/data/collaboration-network.json";
...
export async function generateMetadata(props: {
  ...
  return {
    title: getDict(lang).meta.landscape,
    alternates: langAlternates(lang, "/landscape"),
  };
}

/**
 * The Landscape: first what the machine-labelled subset looks like (category,
 ...
 * over the labelled subset ONLY. The two readings per bucket (any model / all
 ...
 */
export default async function Landscape(props: {
  params: Promise<{ lang: string }>;
}) {
  const params = await props.params;
  const lang: Lang = isLang(params.lang) ? params.lang : "en";
  ...
  const dec = (x: number) =>
    x.toLocaleString(numberLocale(lang), {
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="font-serif text-3xl">{t.landscape.title}</h1>
        <p
      ...
      {/* THE COVERAGE BANNER. It precedes every labelled count on purpose. */}
      <div
        role="note"
      ...
      >
        ...
        {t.landscape.banner(
          n(labelStats.coverage.labeled_works),
          n(labelStats.coverage.frame_works),
          pctLabeled,
        )}
      </div>
      ...
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        />
        <Tile
          label={t.landscape.tileRows}
          value={n(labelStats.coverage.label_rows)}
          note={t.landscape.tileRowsNote}
        />
        <Tile
      ...
      <section className="card p-5">
        <h2 className="font-serif text-xl">
          {t.landscape.collaborationTitle}
        </h2>
        <p
          className="mt-1 max-w-4xl text-sm leading-relaxed"
          style={{ color: "var(--ink-4)" }}
        >
          {t.landscape.collaborationSub(
            n(collaboration.metadata.works_with_named_canadian_institutions),
            n(collaboration.metadata.institutions),
          )}
        </p>
        ...
        <dl className="my-5 grid gap-3 sm:grid-cols-3">
          <div className="rounded-md border p-3">
            <dt className="text-xs uppercase tracking-wider" style={{ color: "var(--ink-4)" }}>
              {t.landscape.collaborationWorks}
            </dt>
            <dd className="tabular mt-1 text-xl">
              {n(collaboration.metadata.works_with_named_canadian_institutions)}
            </dd>
          </div>
          <div className="rounded-md border p-3">
            <dt className="text-xs uppercase tracking-wider" style={{ color: "var(--ink-4)" }}>
              {t.landscape.collaborationInstitutions}
            </dt>
            <dd className="tabular mt-1 text-xl">
              {n(collaboration.metadata.institutions)}
            </dd>
          ...
          <div className="rounded-md border p-3">
            <dt className="text-xs uppercase tracking-wider" style={{ color: "var(--ink-4)" }}>
              {t.landscape.collaborationPeriod}
            </dt>
            <dd className="tabular mt-1 text-xl">
              {t.landscape.collaborationPeriodValue(
                collaboration.metadata.year_from,
                collaboration.metadata.year_to,
              )}
            </dd>
          </div>
        </dl>
        ...
        <CollaborationNetwork
          ...
          worksPath={p("/works")}
          copy={{
            ariaLabel: t.landscape.collaborationAria,
            authorAriaLabel: t.landscape.collaborationAuthorAria,
            institutionMode: t.landscape.collaborationInstitutionMode,
            authorMode: t.landscape.collaborationAuthorMode,
            institutionsShown: t.landscape.collaborationShown,
            authorsShown: t.landscape.collaborationAuthorsShown,
            legendTitle: t.landscape.collaborationLegend,
            authorLegendTitle: t.landscape.collaborationAuthorLegend,
            strongestPairs: t.landscape.collaborationStrongestPairs,
            works: t.landscape.collaborationWorksUnit,
            filterHint: t.landscape.collaborationFilterHint,
            authorHint: t.landscape.collaborationAuthorHint,
            sharedWorks: t.landscape.collaborationSharedWorks,
          }}
        />

        <p className="mt-5 text-xs leading-relaxed" style={{ color: "var(--ink-4)" }}>
          {t.landscape.collaborationMethod(
            n(collaboration.metadata.institution_team_cap),
            n(collaboration.metadata.institution_cap_exclusions),
            n(collaboration.metadata.edge_limit),
            n(40),
          )}
        </p>
        {collaboration.metadata.author_network_status !== "complete" && (
          <p
            className="mt-2 text-xs leading-relaxed"
            style={{ color: "var(--ink-4)" }}
          >
            {t.landscape.authorNetworkPending}
          </p>
        )}
      </section>

      <Frame
        title={t.analytics.byYearTitle}
        note={t.analytics.byYearNote(pctNoAff, n(summary.no_aff))}
      >
      ...
      <Frame
        title={t.analytics.retractionTitle}
        note={t.analytics.retractionNote(n(noticesTotal), n(missedTotal))}
      >
        ...
        <table className="w-full min-w-[560px] text-sm">
          <thead>
            <tr className="border-b" style={{ color: "var(--ink-4)" }}>
              ...
              <th className="p-2 text-right text-xs font-medium uppercase tracking-wider">
                {t.analytics.thFlagged}
              </th>
              <th className="p-2 text-right text-xs font-medium uppercase tracking-wider">
                {t.analytics.thMissed}
              </th>
            </tr>
          </thead>
          <tbody>
            {retractions.map((r) => (
              <tr key={r.nature} className="border-b last:border-0">
                ...
                <td className="tabular p-2 text-right">
                  {n(r.openalex_flagged)}
                </td>
                <td
                  className="tabular p-2 text-right"
                  style={{
                    color: r.openalex_missed ? "var(--retraction)" : "inherit",
                  }}
                >
                  {n(r.openalex_missed)}
                </td>
      ...
      <section className="space-y-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <Tile
            label={t.analytics.tileScored}
            value={n(frameScores.n_scored)}
            note={t.analytics.tileScoredNote}
          />
          <Tile
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.key} className="border-b last:border-0">
              ...
              <td className="tabular p-1.5 text-right">
                {formatInt(lang, r.any)}
              </td>
              <td className="tabular p-1.5 text-right font-medium">
                {formatInt(lang, r.all)}
              </td>
              <td className="p-1.5">
                <div
