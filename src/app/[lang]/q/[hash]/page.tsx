...
import { AUTHOR_LAYER, getPermalink, filtersToQuery, SNAPSHOT, type CanonicalFilters } from '@/lib/permalink'
...
function filterLabel(t: Dictionary, key: string): string {
  const map: Record<string, string> = {
    ...
    author: t.cohort.authorTerm,
    author_id: t.cohort.authorTerm + ' (id)',
    ...
    label_source: t.cohort.labelSource,
    prediction_mode: t.cohort.predictionMode,
...
function filterValue(t: Dictionary, key: string, v: string | number | boolean): string {
  ...
  if (key === 'label_source') return v === 'predicted' ? t.cohort.predictedLabels : t.cohort.directLabels
  if (key === 'prediction_mode')
    return v === 'consensus' ? t.cohort.predictionConsensus : t.cohort.predictionCandidate
...
export default async function QPage({ params }: { params: { lang: string; hash: string } }) {
  ...
  // author_layer is the stamped layer VERSION, not a filter the user chose;
  // it renders in the snapshot section below, not in the filter list.
  const entries = (Object.entries(filters) as Array<[string, string | number | boolean]>).filter(
    ([k]) => k !== 'author_layer',
  )
    ...
    <div className="space-y-8">
      ...
      <section className="card p-6">
        ...
        <p className="mt-4 text-sm">
          <Link href={`${p('/cohort')}${query ? `?${query}` : ''}`} className="link" style={{ color: 'var(--mc)' }}>
            {t.qpage.openBuilder}
          </Link>
        </p>
      </section>
      ...
      <section className="grid gap-4 sm:grid-cols-3">
        ...
        <div className="card p-5">
          <div className="text-xs uppercase tracking-wider" style={{ color: 'var(--ink-4)' }}>
            {t.cohort.predictedLabels}
          </div>
          <div className="tabular mt-2 text-3xl font-semibold" style={{ color: 'var(--contested)' }}>
            {n(predicted)}
          </div>
          <div className="mt-1 text-xs" style={{ color: 'var(--ink-5)' }}>
            {t.cohort.predictionCoverageNote}
          </div>
        </div>
      </section>
      ...
      <section className="card p-6">
        ...
        {filters.author_layer && (
          <p className="mt-2 text-sm leading-relaxed" style={{ color: 'var(--ink-3)' }}>
            {t.qpage.authorLayerLine(String(filters.author_layer))}
            {String(filters.author_layer) !== AUTHOR_LAYER.release && (
              <span style={{ color: 'var(--contested)' }}>
                {' '}
                {t.qpage.authorLayerDrift(AUTHOR_LAYER.release)}
              </span>
            )}
          </p>
        )}
      ...
      <section className="card p-6">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-4 text-sm">
          ...
          <a href={`/api/v1/cohort${query ? `?${query}` : ''}`} className="link break-all font-mono text-xs">
