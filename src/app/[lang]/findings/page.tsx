...
/**
 * The 32 findings, rendered from `findings.json`, the file the pilot scripts
 ...
 * 1.31% because nobody thought to grep for it. Rendering the artifact makes that
  ...
  return function fmt(v: unknown): string {
    if (typeof v === 'number') {
      return Number.isInteger(v)
        ? formatInt(lang, v)
        : v.toLocaleString(numberLocale(lang), { maximumFractionDigits: 20, useGrouping: false })
    }
    if (typeof v === 'boolean') return v ? common.yes : common.no
    if (v === null || v === undefined) return common.none
    return String(v)
  }
}
  ...
  if (Array.isArray(v)) {
    return (
      <ul className="min-w-0 space-y-0.5">
        {v.map((x, i) => (
          <li key={i} className="min-w-0 break-all text-xs">
            {typeof x === 'object' && x !== null ? (
              <code className="whitespace-normal font-mono">{JSON.stringify(x)}</code>
            ) : (
              fmt(x)
            )}
          </li>
    ...
    return (
      <dl className="space-y-0.5">
        {Object.entries(v as Record<string, unknown>).map(([k, x]) => (
          <div key={k} className="flex min-w-0 flex-wrap gap-x-2 text-xs">
            <dt className="break-all" style={{ color: 'var(--ink-5)' }}>
              {k}:
            </dt>
            <dd className="tabular min-w-0 break-all font-medium">{fmt(x)}</dd>
          </div>
        ))}
      </dl>
    )
    ...
    <div className="space-y-8">
      ...
      <div className="space-y-6">
        {keys.map((k, i) => {
          ...
          return (
            <article key={k} className="card min-w-0 p-6">
              ...
              {translated && (
                <p className="mt-3 border-l-2 pl-3 text-xs leading-relaxed" lang="en-CA" style={{ color: 'var(--ink-4)', borderColor: 'var(--border-strong)' }}>
                  ...
                  <dl className="grid gap-x-8 gap-y-2 sm:grid-cols-2">
                    {Object.entries(f.values).map(([vk, vv]) => (
                      <div key={vk} className="min-w-0">
                        <dt className="text-xs" style={{ color: 'var(--ink-4)' }}>
                          <span className="mr-1 uppercase tracking-wider">{t.findings.sourceField}:</span>
                          <code className="break-all font-mono">{vk}</code>
                        </dt>
              ...
              <div className="mt-3 font-mono text-xs" style={{ color: 'var(--ink-4)' }}>
