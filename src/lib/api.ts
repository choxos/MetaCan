...
export function filtersFromParams(sp: URLSearchParams): WorkFilters {
  ...
  const labelSource = s('label_source')
  const predictionMode = s('prediction_mode')
  ...
  return {
    ...
    label_source: (['direct', 'predicted'] as const).find((source) => source === labelSource),
    prediction_mode: (['candidate', 'consensus'] as const).find((mode) => mode === predictionMode),
    labeled: tri('labeled'),
    cited_min: num('cited_min'),
